...
class PastEventCell: UITableViewCell {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if cellNumber == 0 {
            line.move(to: CGPoint(x: eventImage.center.x, y: eventImage.center.y))
        } else {
            line.move(to: CGPoint(x: eventImage.center.x, y: 0))
        }
        
        if cellNumber + 1 == cellsInTotal {
            line.addLine(to: CGPoint(x: eventImage.center.x, y: eventImage.center.y))
        } else {
            line.addLine(to: CGPoint(x: eventImage.center.x, y: bounds.height))
        }
    ...
    func showEvent(name: String) {
        ...
        card.anchor(top: self.topAnchor, leading: self.leadingAnchor, bottom: self.bottomAnchor, trailing: self.trailingAnchor, padding: .init(top: 7, left: 14, bottom: 7, right: 14), size: .init(width: 0, height: 110))
